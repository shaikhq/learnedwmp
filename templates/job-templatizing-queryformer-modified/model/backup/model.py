...
class FeatureEmbed(nn.Module):
    def __init__(self, embed_size=32, tables = 20, types=20, joins = 40, columns= 30, \
                 ops=4, use_sample = False, use_hist = True, bin_number = 50):
    ...
    def forward(self, feature):
        ...
        typeId, joinId, filtersId, filtersMask, hists, tableId = torch.split(feature,(1,1,9,3,self.bin_number*3,1), dim = -1)
    ...
    # changed arg from table_sample to table
...
class QueryFormer(nn.Module):
    def __init__(self, emb_size=32, ffn_dim=32, head_size=8, dropout=0.1,
                 attention_dropout_rate=0.1, n_layers=8, use_sample=False,
    ...
    def forward(self, batched_data, return_embedding=False):
        ...
        # Return single prediction and optional embedding
        pred = self.pred(output[:, 0, :])
        if return_embedding:
