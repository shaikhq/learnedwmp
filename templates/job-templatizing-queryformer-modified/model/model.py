...
class FeatureEmbed(nn.Module):
    def __init__(self, embed_size=32, tables = 10, types=20, joins = 40, columns= 30, \
        ...
        self.project = nn.Linear(1408, embed_size*5 + embed_size//8+1)
       
    
    # input: B by 14 (type, join, f1, f2, f3, mask1, mask2, mask3)
    def forward(self, feature):

        # typeId, joinId, filtersId, filtersMask, table_sample = torch.split(feature,(1,1,9,3,1001), dim = -1)
        typeId, joinId, filterEmbed, filtersMask, hists, tableId = torch.split(feature,(1,1,3*384,3,self.bin_number*3,1), dim = -1)
        
        typeEmb = self.getType(typeId)
        print('typeEmb: ', typeEmb)
        joinEmb = self.getJoin(joinId)
        print('joinEmb: ', joinEmb)
        
        histEmb = self.getHist(hists, filtersMask)
        print('histEmb: ', histEmb)
        print('histEmb shape: ', histEmb.shape)
...
class QueryFormer(nn.Module):
    ...
    def forward(self, batched_data, return_embedding=False):
        ...
        #if return_embedding:
        return self.pred(output[:, 0, :]), second_to_last_embedding
